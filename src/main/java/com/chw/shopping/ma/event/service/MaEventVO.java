...
public class MaEventVO extends CommonVO implements Serializable {
    ...
    private LocalDate strtDt; // 이벤트 시작일

    @NotNull(groups = {insert.class}, message = "이벤트 종료일을 입력해주세요.")
    @DateTimeFormat(pattern = "yyyy-MM-dd")
    private LocalDate endDt; // 이벤트 종료일

    @NotNull(groups = {insert.class}, message = "이벤트 발송 시간을 선택해주세요.")
    @DateTimeFormat(pattern = "HH:mm")
    private LocalTime alertTime; // 이벤트 발송 시간
